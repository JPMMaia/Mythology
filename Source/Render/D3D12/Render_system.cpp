...
namespace Maia::Mythology::D3D12
{
	...
	{
		// TODO create buffer heap
		// TODO create pass buffer
	...
	)
	{
...

		{
			...
			static std::uint8_t created = 0;

			if (created < 3)
			{
				ID3D12Device& device = *m_render_resources.device;

				// TODO create heap one time
				// TODO create one instance buffer per frame
				// TODO give an heap
...

		{
			...
			ID3D12CommandQueue& direct_command_queue = *m_render_resources.direct_command_queue;
...

		{
			...
			std::vector<D3D12_VERTEX_BUFFER_VIEW> instance_buffer_views = // TODO save this and pass to draw
			...
			// TODO should return a command list
			m_upload_frame_data_system.execute(); // TODO Sync copy queue with other render queue
		}

		{
				...
				ID3D12CommandList& command_list =
					m_renderer.render(
						...
						*back_buffer, render_target_descriptor_handle,
						mesh_views,
						instance_buffer_buffer_views,
						pass_data_buffer_address // Owned by render system
					);
...

			{
				...
				ID3D12CommandQueue& direct_command_queue = *m_render_resources.direct_command_queue;
				direct_command_queue.Signal(m_fence.get(), frame_finished_value);
				++m_submitted_frames;
			}
