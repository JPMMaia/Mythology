...
def compile_shaders(
    compiler: pathlib.PurePath, relative_path: pathlib.PurePath, nodes: bpy.types.Node
) -> bool:
    ...
    targets = [
        "%s_%s" % (node.shader_type_property.lower(), node.shader_model_property)
    ...
    compile_commands = [
        [
            ...
            "-Fo",
            str(relative_path / node.output_shader_filename_property),
        ]
    ...
    for command, node in zip(compile_commands, shader_module_nodes):
        print(
            "Compiling shader %s to %s: %s"
            % (
                node.input_shader_file_property,
                node.output_shader_filename_property,
                command,
            )
        )
