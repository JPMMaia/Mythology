...
namespace Maia::Renderer::Vulkan
{
    ...
    std::pmr::vector<Physical_device> enumerate_physical_devices(Instance const instance, std::pmr::polymorphic_allocator<Physical_device> const& allocator) noexcept
    {
        std::uint32_t physical_device_count = 0;
        check_result(
            vkEnumeratePhysicalDevices(instance.value, &physical_device_count, NULL));
        ...
        check_result(
            vkEnumeratePhysicalDevices(instance.value, &physical_device_count, reinterpret_cast<VkPhysicalDevice*>(physical_devices.data())));
